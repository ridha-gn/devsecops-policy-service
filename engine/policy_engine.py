...
class PolicyEngine:
    ...
    def analyze(self, code_type: str, content: str,
                block_threshold: str = 'LOW',
                diff_only: Optional[str] = None) -> PolicyDecision:

        start_time = time.time()
        logger.info(f"Starting analysis for {code_type} code ({len(content)} characters)")
        ...
        try:
            ...
            if code_type == "terraform":
                ...
                normalized_code = self.normalizer.normalize(code_type, parsed_data, extracted_resources)
            elif code_type == "yaml":
                normalized_code = self.normalizer.normalize(code_type, parsed_data, parser=parser)
            else:
            ...
            for policy in self.policies:
                ...
                if violations:
                    logger.info(f"Policy {policy.rule_id} found {len(violations)} violations")
                all_violations.extend(violations)

            # Apply diff filter — keep only violations on changed lines
            if diff_only:
                ...
                if changed_lines:
                    ...
                    logger.info(f"Diff filter applied — {len(changed_lines)} changed lines, {len(all_violations)} violations remain")
            ...
            if allow:
                if all_violations:
                    ...
                    summary = "All security checks passed. Code is compliant with policies."
                logger.info("Analysis result: ALLOW")
            else:
                critical = sum(1 for v in blocking_violations if v.severity.value == "CRITICAL")
                ...
                logger.warning(f"Analysis result: BLOCK ({len(blocking_violations)} violations)")

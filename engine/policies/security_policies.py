...
class RootDockerUserPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        if code.code_type != "dockerfile":
            return violations
        user_instructions = [
            r for r in code.resources if r.resource_type == "docker_user"
        ]
...
class HostNetworkPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        if code.code_type != "kubernetes":
            return violations
        host_network_resources = [
            r for r in code.resources if r.resource_type == "k8s_host_network"
        ]
...
class EC2IMDSv1EnabledPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for instance in instances:
            ...
            http_tokens = (
                metadata.get("http_tokens", "optional")
...
class EC2TerminationProtectionPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for instance in instances:
            ...
            disable_api_termination = instance.get_attribute(
...
class IAMPasswordPolicyPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for policy in password_policies:
            ...
            if (
                min_length < 14
                or not require_uppercase
                or not require_numbers
                or not require_symbols
            ):
                violations.append(
                    PolicyViolation(
                        rule_id=self.rule_id,
        ...
        for sg in security_groups:
            ...
            for rule in ingress_rules:
                if isinstance(rule, dict):
                    ...
                    to_port = rule.get("to_port", 0)
                    cidr_blocks = rule.get("cidr_blocks", [])
                    if (from_port <= 22 <= to_port) and (
                        "0.0.0.0/0" in cidr_blocks or "::/0" in cidr_blocks
                    ):
                        violations.append(
                            PolicyViolation(
                                rule_id=self.rule_id,
...
class DockerLatestTagPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        if code.code_type != "dockerfile":
            return violations
        from_instructions = [
            r for r in code.resources if r.resource_type == "docker_from"
        ]
...
class DockerAddInsteadOfCopyPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        if code.code_type != "dockerfile":
            return violations
        add_instructions = [
            r for r in code.resources if r.resource_type == "docker_add"
        ]
...
class DockerHealthCheckMissingPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        if code.code_type != "dockerfile":
            return violations
        healthcheck_instructions = [
            r for r in code.resources if r.resource_type == "docker_healthcheck"
        ]
...
class DockerSecretsInEnvPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        if code.code_type != "dockerfile":
            return violations
        env_instructions = [
            r for r in code.resources if r.resource_type == "docker_env"
        ]
...
class DockerExposedPrivilegedPortPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        if code.code_type != "dockerfile":
            return violations
        expose_instructions = [
            r for r in code.resources if r.resource_type == "docker_expose"
        ]
...
class DockerNoWorkdirPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        workdir_instructions = [
            r for r in code.resources if r.resource_type == "docker_workdir"
        ]
...
class DockerSecretsInArgPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        if code.code_type != "dockerfile":
            return violations
        arg_instructions = [
            r for r in code.resources if r.resource_type == "docker_arg"
        ]
        ...
        if len(from_instructions) == 1:
            violations.append(
                PolicyViolation(
                    ...
                    line_number=(
                        from_instructions[0].line_number if from_instructions else None
                    ),
                    resource_type="dockerfile",
                    resource_name="FROM",
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for resource in code.resources:
            if "container" in resource.resource_type:
                security_context = resource.get_attribute("securityContext", {})
                allow_escalation = (
                    security_context.get("allowPrivilegeEscalation", True)
                    if isinstance(security_context, dict)
                    else True
                )
                if allow_escalation:
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for resource in code.resources:
            if "container" in resource.resource_type:
                ...
                run_as_user = (
                    security_context.get("runAsUser", 0)
                    if isinstance(security_context, dict)
                    else 0
                )
...
class K8sHostPIDPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        if code.code_type != "kubernetes":
            return violations
        host_pid_resources = [
            r for r in code.resources if r.resource_type == "k8s_host_pid"
        ]
...
class K8sHostIPCPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        if code.code_type != "kubernetes":
            return violations
        host_ipc_resources = [
            r for r in code.resources if r.resource_type == "k8s_host_ipc"
        ]
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for resource in code.resources:
            if "container" in resource.resource_type:
                ...
                if (
                    ":latest" in str(image) or ":" not in str(image)
...
class K8sAutomountServiceAccountPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        if code.code_type != "kubernetes":
            return violations
        automount_resources = [
            r for r in code.resources if r.resource_type == "k8s_automount_token"
        ]
...
class ConfigServiceDisabledPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        violations = []
        config_recorders = code.find_resources_by_type(
            "aws_config_configuration_recorder"
        )
        for recorder in config_recorders:
            status = code.find_resources_by_type(
...
class EKSPublicEndpointPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for cluster in clusters:
            ...
            public_access = (
                vpc_config.get("endpoint_public_access", True)
...
class ECSTaskPrivilegedPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for task in task_definitions:
            ...
            if (
                '"privileged": true' in container_defs
                or '"privileged":true' in container_defs
            ):
                violations.append(
                    PolicyViolation(
                        rule_id=self.rule_id,
...
class ECSTaskReadonlyRootPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for task in task_definitions:
            container_defs = str(task.get_attribute("container_definitions", ""))
            if (
                '"readonlyRootFilesystem": false' in container_defs
                or '"readonlyRootFilesystem":false' in container_defs
            ):
                violations.append(
                    PolicyViolation(
                        rule_id=self.rule_id,
...
class DynamoDBEncryptionPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for table in tables:
            ...
            enabled = (
                sse_spec.get("enabled", False) if isinstance(sse_spec, dict) else False
            )
            if not enabled:
                violations.append(
                    PolicyViolation(
...
class ElastiCacheEncryptionInTransitPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for cluster in clusters:
            transit_encryption = cluster.get_attribute(
...
class ElastiCacheEncryptionAtRestPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        clusters = code.find_resources_by_type("aws_elasticache_replication_group")
        for cluster in clusters:
            at_rest_encryption = cluster.get_attribute(
...
class ECRImageScanningPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for repo in repos:
            ...
            scan_on_push = (
                scan_config.get("scan_on_push", False)
...
class SecretManagerRotationPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        rotation_configs = code.find_resources_by_type(
...
class CloudFrontHTTPSPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for dist in distributions:
            ...
            viewer_protocol = (
                default_cache.get("viewer_protocol_policy", "allow-all")
...
class AzureStoragePublicAccessPolicy(BasePolicy):
    ...
    def check(self, code: NormalizedCode) -> List[PolicyViolation]:
        ...
        for account in accounts:
            public_access = account.get_attribute(
